...
public class McpServer : IAsyncDisposable
{
    ...
    public async Task RunStdioAsync(CancellationToken ct = default)
    {
        ...
        while (!_cts.Token.IsCancellationRequested)
        {
            ...
            try
            {
                ...
                if (response != null) { var json = JsonSerializer.Serialize(response); await writer.WriteLineAsync(json); await writer.FlushAsync(); }
            }
            catch (Exception ex)
            {
    ...
    public async Task<object?> HandleRequestAsync(JsonElement request)
    {
        ...
        {
            case "initialize":
                return new { jsonrpc = "2.0", id = id.GetRawText(), result = new { protocolVersion = "2024-11-05", capabilities = new { tools = new { } }, serverInfo = new { name = "velocity-drone", version = "1.0.0" } } };
            case "tools/list":
                var toolList = GetToolList().Select(t => new { name = t.Name, description = t.Description, inputSchema = GetToolSchema(t.Name) }).ToArray();
            ...
            case "tools/call":
                ...
                var args = request.GetProperty("params").TryGetProperty("arguments", out var argsProp) ? argsProp : JsonDocument.Parse("{}").RootElement;
                ...
                try { var result = await handler(args); return new { jsonrpc = "2.0", id = id.GetRawText(), result = new { content = new[] { new { type = "text", text = result.GetRawText() } } } }; }
                catch (Exception ex) { return new { jsonrpc = "2.0", id = id.GetRawText(), result = new { content = new[] { new { type = "text", text = "Error: " + ex.Message } }, isError = true } }; }
    ...
    {
        "capture_screen" => new { type = "object", properties = new { format = new { type = "string", description = "Image format" } } },
        ...
        "click" => new { type = "object", properties = new { x = new { type = "integer" }, y = new { type = "integer" }, button = new { type = "string" } }, required = new[] { "x", "y" } },
